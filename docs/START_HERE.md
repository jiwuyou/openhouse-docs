...
- Ubuntu 内路径：`/root`、`/root/openhouse/docs`、`/root/openhouseai-docs/official`、`/root/projects`。这是可选兼容工作区；小 App 默认在 Termux native 创建和运行，仅在依赖不兼容时进入 Ubuntu。pi-agent 和 pi-web 默认在 Termux native 层作为长期服务运行。
